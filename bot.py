...
def analyze_target(name, ticker, max_rate, crypto_fng_val):
    try:
        ...
        for price, desc, label in raw_strategies:
            if price >= current:
                safe_price = current - (atr * 0.5)
                if safe_price >= current: safe_price = current * 0.99
                new_desc = desc + " (跌破修正)"
                safe_strategies.append((safe_price, new_desc, label))
            else:
        ...
        # --- V9.4 新增：戰術備註 (Tactical Note) ---
        note = ""
        if best_label == "穩健": # 黃燈
            note = "⚠️ <b>追價提醒：</b>\n"
            note += "1. 雖然趨勢向上，但請<b>分批進場</b> (例如分3份)，不要一次 All-in。\n"
            note += "2. 若遇到「爆大量不漲」或「RSI > 80」，請考慮暫緩或減碼。"
        elif best_label == "保守": # 紅燈
            note = "🛡️ <b>防守提醒：</b>\n"
            note += "1. 目前盤勢偏弱，<b>請嚴格遵守掛單價格</b>，沒買到就算了。\n"
            note += "2. 這是接刀操作，建議<b>預留現金</b>，以防還有更低的低點。"
        else: # 綠燈 (積極)
            note = "⚡ <b>短線提醒：</b>\n"
            note += "1. 這是攻擊型操作，風險較高。\n"
            note += "2. 適合盯盤者，若跌破 10日線 請務必停損。"
        ...
        if is_crypto and max_rate:
            rec_price_str = f"{best_price:.2f} U ({best_price*max_rate:.0f} NT)"
        else:
            rec_price_str = f"{best_price:.1f}"
